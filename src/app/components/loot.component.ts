...
@Component({
    ...
    template: `
        ...
        <div *ngFor="let player of players" class="mb-8 flex flex-col justify-center">
                    ...
                    <div *ngFor="let loot of lootCards">
                        <div class="mx-2 my-1">
                            ...
                            <div class="flex flex-row">
                                <div class="grow">{{ loot.amountOfLoot }}x</div>
                                <button (click)="unLoot(player, loot)">
                                    <img src="assets/trash.svg" class="w-4" />
                                </button>
                            </div>
                            <div class="text-center text-xs">{{ loot.type }}</div>
                        </div>
                    </div>
                </div>
                <div *ngIf="lootCards.length === 0" class=" ml-1 italic font-thin">No loot yet...</div>
                <div *ngIf="deck.length === 0" class=" ml-1 italic font-thin text-red-400">Loot deck is empty.</div>
            </div>
...
export class LootComponent {
    ...
    constructor(private readonly _state: StateService) {
        ...
        this.playerLoot = savedLoot !== null ? this._fromObject(savedLoot.playerLoot) : new Map([]);
    }
    ...
    unLoot(player: string, loot: LootCard): void {
        const i = (this.playerLoot.get(player) ?? []).findIndex(x => x === loot);
        const playerLoot = (this.playerLoot.get(player) ?? []).splice(i, 1);
        this.deck = Utils.shuffle([...this.deck, ...playerLoot]);
        this._save();
    }
    ...
    private _fromObject(obj: Record<string, LootCard[]>): Map<string, LootCard[]> {
