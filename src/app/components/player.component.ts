...
@Component({
    ...
    template: `
        ...
        <form class="mb-4" (submit)="add()">
            ...
            />
            <button type="submit" class="px-4 py-1 rounded-full shadow-sm bg-cyan-500 text-white">Add</button>
        </form>
        <ol class="mb-4 list-decimal list-inside">
            <li *ngFor="let player of players" class="text-lg">{{ player }}</li>
        </ol>
        <div *ngIf="players.length === 0" class="mb-4 ml-1 italic font-thin">Nobody here...</div>
        <div>
