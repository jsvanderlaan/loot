    ...
    template: `
        ...
        <ul class="grid grid-cols-3 gap-2 mb-4">
            <li *ngFor="let type of types" class="p-2 rounded">
                <div class="mx-2 my-1">
                    <button class="disabled:opacity-20 w-full" [disabled]="amount(type) >= max(type)" (click)="add(type)">
                ...
                <div class="flex flex-row">
                    ...
                    <button [disabled]="amount(type) <= 0" (click)="sub(type)" class="px-2 disabled:opacity-20">
        ...
        <div>
            <button type="button" (click)="reset()" class="mr-2 px-4 py-1 rounded-full shadow-sm bg-slate-400 text-white">
                Reset
            </button>
            <button type="button" (click)="done()" class="px-4 py-1 rounded-full shadow-sm bg-cyan-500 text-white">Done</button>
        </div>
    `,
})
export class DeckComponent {
    private readonly _defaultAmounts = new Map(LOOT_TYPES.map(type => [type, 0]));
    private _amounts: Map<Loot, number>;
    private _max: Map<Loot, number>;

    types = LOOT_TYPES;

    constructor(private readonly _state: StateService, private readonly _router: Router) {
        const deck = _state.deck();
        this._amounts =
            deck === null
                ? this._defaultAmounts
                : new Map(LOOT_TYPES.map(type => [type, deck.filter(card => card.type === type).length]));

        this._max = new Map(LOOT_TYPES.map(type => [type, LOOT_CARDS.filter(card => card.type == type).length]));
    }
